...
for ($i=1; $i<=$n; $i++)
{
    ...
    {
       $query = "INSERT INTO pilihan_soal(id_pilihan_soal,id_mata_pelajaran,que,pilihan_a,pilihan_b,pilihan_c,pilihan_d,pilihan_e,ans,userans) VALUES ('$id_pilihan_soal', '$id_mata_pelajaran', '$que', '$pilihan_a', '$pilihan_b', '$pilihan_c', '$pilihan_d', '$pilihan_e', '$ans', '$userans')";
       $hasil = mysqli_query($conn,$query) or die (mysqli_error($conn));

       if ($hasil) echo '<script> alert("Data Berhasil Disimpan");
						location="buatsoal.php?"
						</script>';
